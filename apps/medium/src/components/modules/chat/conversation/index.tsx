...
import { easing, isServer, resonator, useScreenSize } from '@services'
import {
    useCharacter,
    useChat,
    useConversation,
...
export default function Conversation() {
        ...
        <>
            ...
            <footer className="sticky bottom-0 w-full bg-muelsyse pb-4">
                {!user?.username || isUserLoading ? (
                    <a
                        role="article"
                        className="flex items-center gap-3 w-full p-2 rounded-full transition-colors select-none drag-none"
                    >
                    ...
                    <Link
                        href="/profile"
                        role="article"
                        className="sticky bottom-0 flex items-center gap-3 w-full p-2 hover:bg-amber-100 focus:bg-amber-100 rounded-full transition-colors select-none drag-none"
                    >
                        <img
    ...
    return (
        <>
            ...
            {isBig ? (
                <aside
                    className={`z-50 fixed top-0 md:sticky xl:fixed hidden md:flex flex-col items-start w-full md:max-w-xs px-4 bg-muelsyse md:bg-transparent min-h-screen h-screen max-h-screen overflow-y-auto`}
                >
                    {List}
                </aside>
            ) : (
                <AnimatePresence>
                    {showMobileSidebar && (
                        <motion.aside
                            ...
                            className={`z-50 fixed top-0 flex flex-col items-start w-full md:max-w-xs px-4 bg-muelsyse md:bg-transparent min-h-screen h-screen max-h-screen overflow-y-auto shadow-xl`}
                        >
                            {List}
                        </motion.aside>
                    )}
